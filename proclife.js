...
window.setInterval (
	...
	// 半期のグラフ
	if (now.isBefore('2022-9-30')) {
	    app.graphs[3].val = Math.max(0, 100 * (now - dayjs('2022-04-25')) / (dayjs('2022-09-30') - dayjs('2022-04-25')));
	}
	else {
	    app.graphs[3].val = Math.max(0, 100 * (now - dayjs('2022-10-01')) / (dayjs('2023-03-17') - dayjs('2022-10-01')));
	}
	// 今年度のグラフ
	app.graphs[4].val = Math.max(0, 100 * (now - dayjs('2022-04-25')) / (dayjs('2023-03-17') - dayjs('2022-04-25')));
	// プロクラのグラフ
	app.graphs[5].val = Math.max(0, 100 * (now - dayjs('2022-04-01').subtract(annual, 'y')) / (dayjs('2023-03-31') - dayjs('2022-04-01').subtract(annual, 'y')));
    },
    1000
);
